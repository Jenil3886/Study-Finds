...
const SaveContant = ({ activeTab }) => {
	...
	const filteredCaches = SAVE_CACHES.filter((SAVE_CACHES) => SAVE_CACHES.category === activeTab || activeTab === "all");
	const filteredProfiles = SAVE_PROFILES.filter((SAVE_PROFILES) => SAVE_PROFILES.category === activeTab || activeTab === "all");

	return (
		<div className="grid grid-cols-4 m-3 gap-2 select-none">
			{/* Caches Filter.*/}
			{filteredCaches.map((caches) => (
				<div
					key={caches.name}
