...
export const ItemLineCard = ({ item, isLiked, setIsModalOpen }) => {
  const dispatch = useDispatch();

  const likeToggle = (e) => {
    e.preventDefault();
    e.stopPropagation();
    dispatch(likeItem(item.id));
  };
    ...
    >
      ...
      <div className="flex gap-2 w-full items-center justify-between">
        <div className="flex flex-col w-full">
          ...
          {item.description && (
            <p className="text-sm font-hankenMedium overflow-hidden break-words text-gray-500 leading-[22px]">
              {item.description}
            </p>
          )}
          {/* Tags */}
          <div className="flex gap-2 items-center py-1.5">
            {item.tags.map((tag, index) => (
              <div
                key={index}
                className="font-semibold text-[12px] border-2 border-gray-600 rounded-[4px] px-2"
              >
                {tag}
              </div>
            ))}
          ...
          <div className="flex w-full">
            <div className="bg-gray-100 hover:bg-slate-200 text-gray-500 self-start flex items-center font-hankenMedium py-1 px-2 text-[12px] rounded-full overflow-hidden hover:bg-extraGray transition duration-300 gap-x-1">
              ...
              <span className="overflow-hidden whitespace-nowrap text-ellipsis text-[11px]">
                {item.source.name}
              </span>
            </div>
          </div>
        ...
        <div className="max-lg:w-full h-7 flex items-center gap-x-2 lg:gap-x-1.5 2xl:gap-x-3">
          <div className="flex gap-1 items-center">
            ...
            <span className="text-[13px] w-24 xl:w-28 2xl:w-32 overflow-hidden text-ellipsis">
              {item.user.name}
            </span>
          </div>
          <span className="text-[13px] w-[100px] lg:max-2xl:w-[90px] overflow-hidden whitespace-nowrap">
            {item.user.time}
          </span>
